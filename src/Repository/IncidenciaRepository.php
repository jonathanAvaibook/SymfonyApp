...
class IncidenciaRepository extends ServiceEntityRepository
{
    ...
    public function findByCategoryAndTitle($categorySearch, $titleSearch)
    {
        return $this->createQueryBuilder('t')
            ->andWhere('t.Titulo Like :titleSearch')
            ->setParameter('titleSearch', '%'.$titleSearch.'%')
            ->orWhere('t.categoria = :val')
            ->setParameter('val', '%'.$categorySearch.'%')
            ->orderBy('t.id', 'ASC')
            ->setMaxResults(10)
            ->getQuery()
            ->getResult()
